...
import { updateNotice, getNotice } from "../../services/noticeService.js";
...
export default function EditNotice() {
    ...
    useEffect(() => {
        const fetchNotice = async () => {
            try {
                ...
                // Depending on the backend route, it might be res.data or res.data.notice
                const data = res.data.notice || res.data;
                
                // Format publishDate to YYYY-MM-DD
                let formattedDate = data.publishDate;
                if (data.publishDate && data.publishDate.includes('T')) {
                    formattedDate = data.publishDate.split('T')[0];
                }
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                ...
                img.onload = () => {
                    ...
                    setFormData({ ...formData, coverImage: compressedImg });
                };
    ...
    const handleSubmit = async (e, isDraft = false) => {
        ...
        try {
            await updateNotice(id, dataToSave);
        ...
        <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '1rem' }}>
            ...
            <div style={{ backgroundColor: 'white', borderRadius: '1.5rem', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04)', overflow: 'hidden' }}>
                ...
                <div style={{ padding: '1.5rem 2rem', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Update Notice</h2>
                        <p style={{ color: '#6b7280', fontSize: '0.875rem' }}>Modify announcement details for {formData.title || "this notice"}</p>
                    </div>
                    <button type="button" onClick={() => navigate('/notices/view')} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: '#9ca3af' }}>
                        <X size={24} />
                    </button>
                </div>
                            ...
                            <div style={{ position: 'relative', border: '2px dashed #cbd5e1', borderRadius: '0.5rem', padding: formData.coverImage ? '0' : '2rem', textAlign: 'center', cursor: 'pointer', backgroundColor: 'white', overflow: 'hidden' }}>
                                ...
                                {formData.coverImage ? (
                                    ...
                                    <div>
                                        ...
                                        <p style={{ fontSize: '0.875rem', color: '#64748b' }}>Update Cover Image</p>
                                    </div>
                                )}
                            </div>
                ...
                <div style={{ padding: '1.5rem 2rem', borderTop: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'white' }}>
                    <button type="button" onClick={() => navigate('/notices/view')} style={{ padding: '0.75rem 1.5rem', background: 'transparent', border: 'none', color: '#6b7280', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
                    <div style={{ display: 'flex', gap: '1rem' }}>
